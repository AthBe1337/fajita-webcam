...
void AutoFocus::scan_thread(Camera& camera, std::function<float()> get_metric) {
    ...
    for (int i = 0; i < coarse_steps; i++) {
        ...
        if (!move_and_measure(pos, metric))
            return;
        LOG_DEBUG("AF coarse: pos=%d metric=%.1f", pos, metric);
    ...
    // Phase 2: Fine scan (±128 around best, step 32)
    {
        ...
        for (int pos = fine_min; pos <= fine_max; pos += 32) {
            ...
            if (!move_and_measure(pos, metric))
                return;
            LOG_DEBUG("AF fine: pos=%d metric=%.1f", pos, metric);
            if (metric > best_metric) {
                best_metric = metric;
                best_pos = pos;
            }
    ...
    if (!camera.set_focus(best_pos)) {
        LOG_WARN("AF: final move failed at pos=%d", best_pos);
        state_ = AFState::Failed;
        locked_metric_ = 0;
        return;
    }
    ...
    LOG_DEBUG("AF: locked at pos=%d metric=%.1f", best_pos, best_metric);
...
void AutoFocus::update_continuous(Camera& camera, float metric) {
    ...
    if (locked_metric_ > 0 && metric < locked_metric_ * 0.7f) {
        LOG_DEBUG("AF continuous: contrast dropped (%.1f -> %.1f), re-scanning",
        ...
        for (int pos = std::max(0, cur - range);
            ...
            if (!camera.set_focus(pos)) {
                LOG_WARN("AF continuous: move failed at pos=%d", pos);
                state_ = AFState::Failed;
                locked_metric_ = 0;
                return;
            }
        ...
        if (!camera.set_focus(best_p)) {
            LOG_WARN("AF continuous: restore failed at pos=%d", best_p);
            state_ = AFState::Failed;
            locked_metric_ = 0;
            return;
        }
